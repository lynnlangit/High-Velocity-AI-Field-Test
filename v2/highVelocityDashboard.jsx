...
export default function HighVelocityDashboard() {
  ...
  const fetchGeminiGuidance = async () => {
    ...
    // --- MOCK MODE (If no API Key or API Error) ---
    const generateMockGeminiMessage = () => {
      // Idle messages removed to reduce chattiness as requested
      ...
      if (currentData.speed > 135) {
        ...
        return null; // Silence Gemini if no specific advice
      }
    };

    if (!apiKey) {
      await new Promise(r => setTimeout(r, 800)); // Simulate latency
      const mockMsg = generateMockGeminiMessage();
      if (mockMsg) addLog(mockMsg);
    ...
    try {
      ...
      // FALLBACK TO MOCK ON ERROR (To save the demo)
      // Only log aggressive error if it's NOT a 404/Auth error to avoid spam
      ...
      if (!isAuthOrNotFoundError) {
        addLog({
          ...
          priority: "normal" // Downgraded to normal to not block Nano
        });
      }

      // Generate a valid mock response so the user hears *something*
      const fallbackMsg = generateMockGeminiMessage();
      if (fallbackMsg) addLog(fallbackMsg);
